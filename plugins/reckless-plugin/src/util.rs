...
pub async fn run_logged_command(
    ...
    let line = format!("running command: `{command:#?}`");
    ...
    if !status.success() {
        return Err(anyhow!("command failed: {command:#?}, status={status}"));
    }
...
pub async fn read_sources_file(
    plugin: Plugin<PluginState>,
) -> Result<(Vec<Url>, Vec<PathBuf>, PathBuf), anyhow::Error> {
    ...
    for line in &lines {
        if let Ok(url) = Url::from_str(line) {
            ...
            paths.push(PathBuf::from_str(line)?);
        }
    }
...
pub async fn search_sources(
    ...
    let (urls, paths, _source_file) = read_sources_file(plugin.clone()).await?;
    let mut rl_plugins = HashMap::new();

    for url in &urls {
        let repo_dir = init_plugin_repo(plugin.clone(), url, logger).await?;
        rl_plugins.extend(
            find_plugin_locs(
