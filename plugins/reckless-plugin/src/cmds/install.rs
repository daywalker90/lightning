...
use crate::{
    ...
    structs::{
        InstallArgs, InstallResponse, Installer, PluginState, RecklessLogger, RecklessPlugin,
...
pub async fn handle_install(
    ...
    match install_result {
        ...
        Err(e) => {
            let line = format!("{plugin_name} install failed: {e}");
            logger.log(&line, LogLevel::UNUSUAL).await?;
...
pub async fn handle_uninstall(
    ...
    match uninstall(plugin.clone(), plugin_name.clone(), &mut logger).await {
