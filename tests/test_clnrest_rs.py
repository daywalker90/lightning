...
def get_clnrest_rs_bin():
    return Path(__file__).resolve().parent.parent / "plugins" / "clnrest-rs"
...
def test_clnrest_no_auto_start(node_factory):
    ...
    l1 = node_factory.get_node(
        options={'disable-plugin': 'clnrest', 'plugin': get_clnrest_rs_bin()}
    )
    # This might happen really early!
    l1.daemon.logsearch_start = 0
    assert [p for p in l1.rpc.plugin('list')['plugins'] if 'clnrest' in p['name']] == []
    assert l1.daemon.is_in_log(r'plugin-clnrest-rs: Killing plugin: disabled itself at init: `clnrest-port` option is not configured')
...
def test_clnrest_self_signed_certificates(node_factory):
    ...
    l1 = node_factory.get_node(options={'disable-plugin': ['cln-grpc', 'clnrest'],
                                        'plugin': get_clnrest_rs_bin(),
                                        'clnrest-port': rest_port,
                                        'clnrest-host': rest_host})
    # This might happen really early!
    l1.daemon.logsearch_start = 0
    l1.daemon.wait_for_log(r'plugin-clnrest-rs: REST server running at ' + base_url)
...
def test_clnrest_uses_grpc_plugin_certificates(node_factory):
    ...
    l1.daemon.wait_for_logs([r'serving grpc on 0.0.0.0:',
                             r'plugin-clnrest-rs: REST server running at ' + base_url])
    ca_cert = Path(l1.daemon.lightning_dir) / TEST_NETWORK / 'ca.pem'
    http_session = http_session_with_retry()
    response = http_session.get(base_url + '/v1/list-methods', verify=ca_cert)
    assert response.status_code == 200
...
def test_clnrest_generate_certificate(node_factory):
    ...
    l1 = node_factory.get_node(options={'disable-plugin': 'clnrest',
                                        'plugin': get_clnrest_rs_bin(),
                                        'clnrest-port': rest_port,
...
def test_clnrest_numeric_msat_notification(node_factory):
    ...
    l1, l2 = node_factory.get_nodes(2, opts=[{'disable-plugin': ['cln-grpc', 'clnrest'],
                                              'plugin': get_clnrest_rs_bin()},
                                             {'disable-plugin': ['cln-grpc', 'clnrest'],
                                              'plugin': get_clnrest_rs_bin(),
                                              'clnrest-port': rest_port,
...
def test_clnrest_options(node_factory):
    ...
    l1 = node_factory.get_node(options={'disable-plugin': ['cln-grpc', 'clnrest'],
                                        'plugin': get_clnrest_rs_bin(),
                                        'clnrest-port': rest_port})
    assert l1.daemon.is_in_log(f'plugin-clnrest-rs: Killing plugin: disabled itself at init: `clnrest-port` {rest_port}, should be a valid available port between 1024 and 65535.')
    ...
    l1 = node_factory.get_node(options={'disable-plugin': ['cln-grpc', 'clnrest'],
                                        ...
                                        'clnrest-port': rest_port,
                                        'clnrest-protocol': rest_protocol})
    assert l1.daemon.is_in_log(r'plugin-clnrest-rs: Killing plugin: disabled itself at init: `clnrest-protocol` can either be http or https.')
    ...
    l1 = node_factory.get_node(options={'disable-plugin': ['cln-grpc', 'clnrest'],
                                        ...
                                        'clnrest-port': rest_port,
                                        'clnrest-host': rest_host})
    assert l1.daemon.is_in_log(r'plugin-clnrest-rs: Killing plugin: disabled itself at init: `clnrest-host` should be a valid IP.')
...
def test_clnrest_http_headers(node_factory):
    ...
    # This might happen really early!
    l1.daemon.logsearch_start = 0
    l1.daemon.wait_for_log(f'plugin-clnrest-rs: REST server running at {base_url}')
    ...
    l2.daemon.wait_for_log(f'plugin-clnrest-rs: REST server running at {base_url}')
    ca_cert = Path(rest_certs) / 'ca.pem'
...
def test_clnrest_old_params(node_factory):
    ...
    l2 = node_factory.get_node(options={'disable-plugin': ['cln-grpc', 'clnrest'],
                                        'plugin': [get_clnrest_rs_bin(), plugin],
                                        'rest-port': rest_port,
                                        'rest-host': rest_host,
                                        'allow-deprecated-apis': True},
                               broken_log=r'DEPRECATED API USED rest-*')
    ...
    assert l2.daemon.is_in_log(r'plugin-clnrest-rs: Killing plugin: disabled itself at init: `clnrest-port` option is not configured')
